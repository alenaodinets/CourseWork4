...
class ApiChange(AbstractApi):
    """API, который отвечает за перевод иностранной валюты
    ...
    def _make_params(self):
        ...
        params = {'symbol': self.symbols, 'base': self.__base}
        return params

    def _make_header(self):
        """Заголовок для запроса"""
        header = {'apikey': self.__api}
        return header
    ...
    @ex_change.setter
    def ex_change(self, value):
        self._ex_change = value


class MixinConvert(BaseModel, extra='allow'):
    """Внедрение Exchange в основную программу"""
    convert_to_RUB: bool = False

    def _make_list_for_convert(self, list_: List[Dict]):
        """Проходится по списку объектов и выводит иностранные валюты
        Args:
    ...
    def get_rate_currency(self, list_: List[str]) -> Dict:
        ...
        Returns:
            Dict: Возвращает результат запроса с индексами указанных валют"""
    ...
    def _convert_valute(self, list_: List[Dict]) -> List[Dict]:
        """Проходится по списку с объектами и меняет
        значение денежного эквивалетна у иностранной валюты
        Args:
...
class MixinSort(BaseModel, extra='allow'):
    """Сортировка объектов по определенному значению"""
...
class MixinTown(BaseModel, extra='allow'):
    ...
    def make_id_of_town(self, town: str) -> int:
        """Получает имя города и возвращает ID
        Args:
            town (str): имя города
        Returns:
            int: ID города"""
...
class HhVacancies(MixinTown, MixinSort, MixinConvert, AbstractApi):
    ...
    def model_post_init(self, __context: Any) -> None:
        """Меняет состояние выходящего запроса исходя из параметров"""
    ...
    def _remove_null_instanse(self, list_: List[Dict]):
        """Переработка объектов в запросе и замена None на 0 в salary
        Args:
